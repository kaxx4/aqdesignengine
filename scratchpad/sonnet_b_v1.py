...
async def main():
    ...
    sign_x, sign_w = 140, 560          # outline sign frame
    pad = 40
    text_max_w = sign_w - 2 * pad      # 480

    sizes = await B.measure_text([
        dict(text=HERO_TXT, font="d", size=280, weight=900, line_height=0.82,
             transform="uppercase", max_width=text_max_w),
    ...
    leg_x, leg_y, leg_w, leg_h = 520, 232, 420, 720
    ...
    sub_x = sign_x + pad
    sub_y = hero_y + hero_m["h"] + 18
    sub_html = (f'<div class="measure" data-tag="sub" style="position:absolute;top:{sub_y}px;'
                f'left:{sub_x}px;width:{text_max_w}px;font-family:var(--e);font-weight:600;'
    ...
    col_x, col_w = 140, 800   # same left inset as the sign, right inset matching the leg's edge
    ...
    cta_y, cta_h = code_y + code_h + 14, 76
    ...
    fine_html = (f'<div class="measure" data-tag="fine" style="position:absolute;top:{fine_y}px;'
                 ...
                 f'font-weight:400;font-size:20px;line-height:1.5;color:var(--ink3);z-index:9">'
                 f'Share your code with a friend to open a seat<br>in an upcoming aquaterra '
                 f'workshop.</div>')
    elements.append(("fine", col_x, fine_y, col_w, 60))
    ...
    out_png = os.path.join(out_dir, "B_v1.png")
