...
# v5: the upper-right region (v3/v4's 3 worst REGION critiques) is not a smooth
# sine bend at all — zoomed crops (scratchpad/crop_zoom1.png, crop_zoom2.png) show
# the reference ribbon making a quarter-turn from vertical to a flat horizontal
# run, THEN a tight ~180-degree hook hugging the right edge, THEN another flat
# run back left. A sparse Catmull-Rom through 5 points forced that into a single
# diagonal, which is why v3/v4 were too narrow exactly where the reference is a
# wide near-horizontal band (compare.py's "REGION UNDER-filled row3/col6" and
# "OVER-filled row4/col8"). Denser points describing the actual hook shape:
# v6: whole hook shifted ~70px earlier (smaller y) — v5's remaining worst cells
# (row2/col7 UNDER, row4/col6-7 OVER) both point the same direction: the flat
# rightward band and the hook itself sit noticeably HIGHER in the reference
# (visible in crop_zoom1: the band is already flat by crop-y0-40 = full y60-100)
# than v5 modeled (v5's flattening only started at y160-175).
WAYPTS_SRC = [(681, -40), (650, 30), (670, 70), (820, 90), (960, 95), (1090, 90),
              (1165, 105), (1180, 140), (1165, 175), (1080, 200), (950, 210),
              (820, 220), (700, 245),
              (661, 400), (638, 420), (635, 440),
...
async def main():
    ...
    await B.render(html, f"out/versions/{SLUG}/v6.png", W, H, elements=elements,
                   color_pairs=color_pairs, page_bg="var(--bg)", expect_hero=True,
                   collision_ignore=COLLISION_IGNORE, bleed_tags=("ribbon",))
    print("done -> out/versions/%s/v6.png" % SLUG)
