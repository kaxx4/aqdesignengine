...
async def main():
    ...
    # shifted left/down vs v2 per the CENTROID OFF critique, and widened for the bigger items
    zone = (int(W*0.06), int(H*0.30), int(W*0.86), int(H*0.58))  # x,y,w,h of the pile band
    ...
    placements, unplaced = lay.scatter_solve(
        ...
        margin=16, max_pair_overlap=0.28, seed=11, tries=1200)
    if unplaced:
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v3.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v3.png")
