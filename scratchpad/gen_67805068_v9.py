...
67805068493b4522f5c3944723bee7d2.jpg), v9 -- FINAL.

V9 FIXES OVER V8 (score 0.164 -> under the 0.16 accept line): three more small region deltas,
all magnitude ~0.32, the smallest yet -- enlarging fillerGap to actually reach col4 (it was
only grazing col4/5's boundary), shrinking/raising flowerPink slightly (it was over-filling
row5,col8), and nudging flowerBrown/kiteBlueLt to plug row8,col7's remaining gap. Diminishing
returns from here per §5 ("most took 1-3 iterations" -- this one needed more because v1-v3's
wrong canvas meant it effectively started from a full rebuild, per the STATUS note below).

V8 FIXES OVER V7 (score 0.168 -> target <=0.16, final pass): OVER-filled row9/11,col4/9 --
the AQUATERRA wordmark reads noticeably bolder/larger than the reference's own PLAYBOOK
lockup (compare.crop confirmed this back in v5); shrunk further. UNDER-filled row6/11,
col4/9 and col5/9 -- a real central gap between the left sticker cluster and the
"someone/else" cluster that no existing element reaches; per the decision table ("only add
filler as a last resort" -- four iterations of moving existing elements had already been
tried on the OTHER gaps and worked, so this is the legitimate last resort for the one gap
nothing else reaches), one small connecting sticker added there, in the same ring vocabulary.

V7 FIXES OVER V6 (score 0.176 -> target <=0.16): three more region deltas from compare.report:
UNDER-filled row3/11,col3/9 and row4/11,col3/9 (a real gap between flowerA/blobPillow and
bunnyStretch that the reference fills) -- flowerA enlarged/shifted right, blobPillow enlarged,
bunnyStretch shifted to close the gap. OVER-filled row6/11,col9/9 -- animalPink had drifted too
far right/down chasing v5's under-fill at the SAME edge; pulled back up-left and shrunk slightly
so it no longer reaches that far-right cell at all.

V6 FIXES OVER V5 (score 0.181 -> target <=0.16): compare.report on v5 flagged (a) OVER-filled
row8/11,col8/9 -- flowerBrown+kiteBlueLt too dense/large in that cell, shrunk+separated;
(b) UNDER-filled row5/11,col9/9 -- flowerPink/animalPink had been pulled too far inward fixing
v4's off-canvas overflow, pushed back toward the (still on-canvas) edge; (c) OVER-filled
row3/11,col2/9 -- flowerA slightly too big for that cell, shrunk a touch; (d) MISSING COLOUR
orange/warm rgb(160,96,0) -- BROWN_MD swapped from a lemon-only derivation to a lemon/tomato
blend, landing much closer to the measured target hue while still being accent-derived.
...
# v6: measured MISSING COLOUR critique wanted rgb(160,96,0) (warm orange-brown); a pure
# lemon-derived mustard read too olive. Blending in tomato pulls it toward the true warmth.
BROWN_MD = _blend(BROWN_DK, core.ink_of(A[3]), 0.45)   # ~orange-brown, still accent-derived
...
def at(x, y, w, h, inner, z=10, rot=0, label=None, tag=None):
    if label:
        elements.append((label, x, y, w, h))
    r = f"transform:rotate({rot}deg);" if rot else ""
    dt = f' data-tag="{tag}"' if tag else ""
    return (f'<div{dt} style="position:absolute;left:{x:.0f}px;top:{y:.0f}px;width:{w:.0f}px;'
            f'height:{h:.0f}px;z-index:{z};{r}">{inner}</div>')


def stick(shape_path, fill, x, y, size, rot=0, box=100, label=None, z=20, tag=None):
    ...
    return at(x, y, size, size, svg, z=z, label=label, tag=tag)
...
async def build():
    ...
    ring = [
        ...
        ("flowerA",      S.blob(seed=3, lobes=8, wobble=0.36),                 A[1], 0.13, 0.15,  0.19,  -8,  22),
        ...
        ("blobPillow",   S.blob(seed=7, lobes=6, wobble=0.12),                 A[4], 0.03, 0.255, 0.185, -8,  20),
        ("kiteYellow",   S.starburst(points=4, R=50, r=16),                    A[2], 0.665,0.27,  0.13,   0,  22),
        ("shieldYellow", S.shield(),                                          A[2], 0.775,0.285, 0.16,   8,  18),
        ("bunnyStretch", S.capsule(w=100, h=70),                              CREAMY,0.24, 0.305, 0.34,  -24, 26),
        ...
        ("flowerPink",   S.scallop(lobes=9, r=44),                             A[0], 0.825,0.365, 0.15,   0,  20),
        ("animalPink",   S.capsule(w=100, h=60),                              A[0], 0.70, 0.42,  0.17,   16,  22),
        ...
        ("flowerBrown",  S.scallop(lobes=8, r=44),                            BROWN_DK,0.765,0.635,0.125,-6,  20),
        ("kiteBlueLt",   S.starburst(points=4, R=50, r=16),                    A[4], 0.665,0.675, 0.13,   6,  22),
        ...
        ("fillerGap",    S.starburst(points=5, R=48, r=22),                    A[3], 0.375,0.465, 0.115, 15,  20),
    ]
    for label, path, fill, xf, yf, sf, rot, z in ring:
        ...
        if path is None:
            ...
            tag = "bleed" if label == "starRedBig" else None
            els_html.append(stick(path, fill, x, y, s, rot=rot, label=label, z=z, tag=tag))
    ...
    wx, wy = F(0.35, 0.765)
    tw = 0.30 * W
    els_html.append(at(wx, wy, tw, 170,
        f'<div style="font-family:var(--d);font-weight:900;font-size:42px;line-height:1.08;'
    ...
    bleed_tags = ("bleed",)   # matches the data-tag="bleed" on starRedBig only (audit.py /
                              # measure_dom match on data-tag, a DIFFERENT namespace from the
                              # python-side element labels below -- see friction doc)
    ...
    collision_ignore = {
        ...
        ("shieldYellow", "flowerPink"), ("bunnyStretch", "fillerGap"),
        ("stemGreenPink", "animalPink"), ("fillerGap", "w_show"),
        ("spiralYellow", "bearBrown"), ("spiralYellow", "w_up"),
    }

    slug = "67805068493b45"
    os.makedirs(f"out/versions/{slug}", exist_ok=True)
    out_png = f"out/versions/{slug}/v9.png"
