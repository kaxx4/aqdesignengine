...
# ---------------------------------------------------------------------------
CARD_A = dict(x=54,  y=210, w=610, h=560)   # headline / hero stat -- grape, frontmost
CARD_B = dict(x=560, y=330, w=430, h=480)   # accent stat -- tomato, behind+right of A
CARD_C = dict(x=40,  y=650, w=520, h=560)   # photo 1 -- Sundarban education photo
CARD_D = dict(x=500, y=730, w=520, h=580)   # photo 2 -- food distribution photo
...
async def build():
    ...
    items = [
        ...
        dict(text="291", font="d", size=140, weight=900),
        dict(text="WORKSHOPS RUN", font="d", size=30, weight=900, max_width=360),
    ...
    elements.append(("num558", CARD_A["x"]+24, CARD_A["y"]+92, numA_w, numA_h))
    text_pairs += [
    ...
    b_inner = (
        f'<div style="position:absolute;left:28px;top:{28}px;font-family:var(--d);font-weight:900;'
        f'font-size:140px;line-height:.8;color:{TXT_ON_TOMATO}">291</div>'
        f'<div style="position:absolute;left:28px;top:{28+m291["h"]+4}px;width:360px;font-family:var(--d);'
        f'font-weight:900;font-size:30px;line-height:1.05;color:{TXT_ON_TOMATO}">WORKSHOPS RUN</div>'
        f'<div style="position:absolute;left:28px;top:{28+m291["h"]+4+mWORK["h"]+8}px;font-family:var(--e);'
        f'font-weight:600;font-size:19px;color:{TXT_ON_TOMATO}">same span</div>'
    )
    elements.append(("num291", CARD_B["x"]+28, CARD_B["y"]+28, m291["glyph_w"], m291["glyph_h"]))
    text_pairs += [
        ("num291", TXT_ON_TOMATO, TOMATO, 140, True),
        ("WORKSHOPS_RUN", TXT_ON_TOMATO, TOMATO, 30, True),
        ("same_span", TXT_ON_TOMATO, TOMATO, 19, True),
    ]
    b_block_bottom = 28 + m291["h"] + 4 + mWORK["h"] + 8 + msame["h"]
    contains_pairs.append(
        ("stat_block_in_cardB", (CARD_B["x"]+28, CARD_B["y"]+28, 360, b_block_bottom-28+16),
         (CARD_B["x"], CARD_B["y"], CARD_B["w"], CARD_B["h"])))
    ...
    badge_size = 168
    badge_x, badge_y = CARD_A["x"]+CARD_A["w"]-64, CARD_A["y"]+90
    ...
    logo_pill = (f'<div class="measure" data-tag="logo" style="position:absolute;left:48px;top:56px;'
    ...
    elements.append(("logo", 48, 56, 160, 46))
    kicker = (f'<div class="measure" data-tag="kicker" style="position:absolute;right:48px;top:66px;'
    ...
    elements.append(("kicker", W-48-260, 66, 260, 24))
    text_pairs.append(("kicker_text", core.on_dark(LEMON, 16), BG, 16, True))

    # ---- footer ----
    foot1 = (f'<div class="measure" data-tag="foot" style="position:absolute;left:48px;bottom:96px;'
    ...
    elements.append(("foot1", 48, H-96-16, 400, 18))
    foot2 = (f'<span class="measure" data-tag="cta" style="position:absolute;bottom:{56}px;left:48px;'
    ...
    elements.append(("foot2", 48, H-56-18, 320, 18))
    ...
    async with B.session():
        issues = await B.render(html, f"{outdir}/g4_v1.png", W, H,
