...
async def main():
    ...
    zone = (int(W*0.09), int(H*0.31), int(W*0.72), int(H*0.50))  # x,y,w,h — tighter than v3
    ...
    placements, unplaced = lay.scatter_solve(
        ...
        margin=12, max_pair_overlap=0.36, seed=23, tries=1500)
    if unplaced:
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v4.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v4.png")
