"""2022ebef4ffad5 v4 — full rebuild (v1-3 script lost).
Reference: red-flooded party flyer. Repeated "FRIDAY 24TH" card deck (cascading, decreasing
scale/increasing rotation), disco ball + sparkles top-right of deck, drink glasses flanking,
teal/blue envelope triangle directly under the deck (NO gap), elephant head + paw bottom-right,
wine/spray bottle bottom-left, RSVP block bottom-left (must not collide with the bottle).

v3 critique (compare.py): DETAIL TOO LOW 0.63x, region under-filled row8 col4-6 (the dead gap
between card deck and envelope), missing near-neutral grey (disco ball too small/absent).
Fixes here: cards pushed closer to headline + taller so the deck's bottom meets the envelope's
tip with no gap; each card carries a dense row of stick figures + motion lines (detail); a real
...
headline = (f'<div style="position:absolute;top:150px;left:0;right:0;text-align:center;z-index:70;color:#0A0A0A;font-family:var(--m)">'
...
elements.append((240, 150, 600, 190))
...
hydration = at(M, env_top + 130, 84, 104,
               ...
               f'font-family:var(--m);font-weight:700;font-size:10px;letter-spacing:.04em">AQ<br>SPRAY</div></div>', z=19)
