...
CARD_BG = "#D2CEC4"                  # warm light-grey "paper" -- see note below.
                                      ...
                                      # gate/reference mismatch, noted in the friction report. v2's
                                      # fix (a light AQ-grape wash, dist 41.5) cleared the gate but
                                      # read as an unrelated purple tint with no basis in the
                                      # reference. This warm light grey (dist 55.0) reads closer to
                                      # an actual "paper" tone while still clearing the gate with
                                      # real margin.
INK = core.INK
GREY = "#6B6B66"
...
def clipped(label, x, y, w, h, clip_poly, fill=None, photo=None, z=1, bg_override=None):
    bg = bg_override or (f'url({photo}) center/cover' if photo else fill)
    parts.append(
...
# v4's left-edge bleed (this panel + BL below) ran continuously from y157 clear to the
# canvas bottom (628) -- compare.py's grid measurement showed the reference's actual left
# edge is a single band roughly y285-513 (rows 5-8 of 11) with NOTHING at the very top
# (rows 0-4) or very bottom (rows 9-10, including the row11/col1 "OVER-filled, reference
# 0.00" critique on v1-v4). Moved down and shortened to match.
clipped("bleed_lmid_person", 240, 280, 60, 110, "40% 0%, 100% 10%, 100% 100%, 0% 90%",
        photo=core.PHOTOS["xmas"], z=1)
# 5. BL book-pages corner -- flat pale-sky panel (swap: stock photo with foreign-language type)
# shortened for the same reason -- v4 filled the literal bottom-left CORNER solid, which
# compare.py flagged every version as "REGION OVER-filled row 11/11 col 1/9 (reference 0.00)".
# v6's rectangle-with-corner-cut was still FULL WIDTH for its bottom two-thirds, so row 9/11
# (the row right above the dead corner) came back at 1.00 vs the reference's 0.32 -- the
# reference's own column-1 profile rises to a peak around row 6-7 then TAPERS toward the
# bottom, i.e. a real wedge, not a rectangle with one corner clipped. Re-shaped to taper.
# compare.py's grid showed the reference's left edge actually starts filling around row 7
# (y342), 48px above where this panel started (y390) -- extended upward to match; kept the
# taper shape and bottom edge the same.
clipped("bleed_bl_pages", 0, 345, 156, 155, "0% 0%, 100% 16%, 28% 100%, 0% 100%",
...
# compare.py's grid showed rows 0-1 (y0-114) at 1.00/0.66 but row 2 (y114-171) dropping to
# 0.01 in the reference -- a much sharper cutoff than this panel's diagonal (height 188) was
# giving (this render still showed 0.70 there). Shortened so the diagonal resolves higher up.
clipped("bleed_tr_tulip", 954, 0, 246, 120, "22% 0%, 100% 0%, 100% 100%, 0% 42%",
        photo=core.PHOTOS["diwali"], z=1)
# 8. R-mid buttercup -- real AQ photo (edu), largest bleed panel.
# v4 touched the canvas's own right edge (x1200) across this panel's full height (y302-559,
# rows 4-8); v5/v6 narrowed it to end at x1150, which still crossed into col 9/9 -- compare.py
# kept flagging that column as OVER-filled (reference col 9/9 is 0.00 for this whole middle
# band; it only has content at the very top, the tulip corner rows 0-2, and the very bottom,
# the swing tag rows 9-10). Pulled fully clear of col 9 this time (right edge now 1067, the
# col8/col9 boundary) and shifted left to increase overlap with col 8, where the reference
# DOES have real content (0.35-0.76 across rows 5-8) that v6 under-filled.
# v7 still ran this panel down to y559, reaching into row 9/11 (y513-570) where the
# reference is nearly empty (0.05) -- the reference's real vertical extent for this bleed is
# roughly rows 4-8 (y228-513); shortened to stop at y505.
clipped("bleed_rmid_flowers", 960, 302, 107, 203, "35% 0%, 100% 0%, 100% 100%, 0% 100%",
        photo=core.PHOTOS["edu"], z=1,
        # core.PHOTOS['edu'] ALSO carries a baked-in caption ("CREATING MOMENTS ETCHED IN
        # THEIR HEARTS FOREVER") in its own bottom band -- missed in v1-v10 because this
        # panel's narrow/tall aspect (0.53) vs the source's 0.8 means a plain center/cover
        # crop shows the photo's full height with no room to crop the bottom (same root cause
        # as the food-photo caption fix on the book-photo tile, just not caught here until a
        # visual look at v10.png). Same fix: oversized top-anchored background-size.
        bg_override=f'url({core.PHOTOS["edu"]}) top center / 216px 270px no-repeat')
# 10. bottom-mid chair -- flat tomato panel (swap: small stock furniture photo). The audit
# step-1 estimate (144x38) badly under-measured this: a direct crop of the reference bottom
# edge (scratchpad/_ref_bottom_chair.png, x550-900 y540-628) shows the chair image is one of
# the WIDER bleed panels, not a thin sliver -- confirmed by compare.py, which kept flagging
# row 11/11 (the bottom canvas edge) as under-filled (reference 0.76, this render 0.16) even
# after every other fix. A first widen (x550-900) overshot the other direction -- the
# reference's row-11 grid is exactly 0.53/0.76 at columns 5-6 (x533-800) and 0.00 at column 7
# (x800-933), so the true right edge sits at 800, not 900. Narrowed back to that span.
clipped("bleed_bmid_chair", 590, 578, 190, 50, "12% 0%, 88% 0%, 96% 100%, 4% 100%",
        fill=A[3], z=1)
...
# v1-v7 started this at y462, whose rotated footprint spilled up into row 8/11 (y456-513),
# a row compare.py's grid says the reference leaves empty at the far right (col 9/9 is 0.00
# there) -- the reference's tag+corner content only begins around row 9/11 (y513). Shifted
# down and shrunk slightly to stay clear while still landing fully on-canvas.
TAG_X, TAG_Y, TAG_W, TAG_H, TAG_DEG = 1005, 495, 150, 115, -9
...
step("Production", "02", 452, 138)
E("step_production", 452, 138, 155, 22)
step("Review", "03", 372, 380)
E("step_review", 372, 380, 115, 22)
step("Delivery", "04", 752, 384)
E("step_delivery", 752, 384, 110, 22)
...
# v4 made this RH=200 to read as a real object rather than v3's 25px garnish sliver -- but
# that satisfied the LOOKING gate at the cost of the SCORE: compare.py's grid showed the
# reference has almost no content (0.04-0.08) at x667-800,y457-514, which v4/v5's 200px-tall
# rect filled at 0.85-0.89. Re-measured directly off the reference crop
# (scratchpad/_ref_pile_full.png, offset 550,50): the green sliver is only visible from
# about crop-y295 to crop-y365, i.e. absolute y345-415 -- 70px, not 200. v3's instinct on
...
# 20. blue book photo -- re-measured off the same crop: crop-local x45-235,y108-330 ->
# absolute x595-785,y158-380 (190x222). v1-v5 had this 290px tall (to y465), overshooting the
# reference's actual bottom edge (380) by 85px and eating the "empty gap above the pile"
# rhythm the reference has between the sticky-note stack and the book/Aa/Au cluster below it.
BX, BY, BW, BH = 595, 158, 190, 222
...
# 23. lime "Au" tile -- overlaps Aa's lower-left corner + book photo top. Re-measured off
# the crop: crop-local x100-205,y155-260 -> absolute x650-755,y205-310.
UX, UY, UW, UH = 650, 205, 105, 105
PILE_BOX["autile"] = (UX, UY, UW, UH)
LABEL_BOX["autile"] = (UX + UW * 0.30, UY + UH * 0.30, UW * 0.40, UH * 0.40)
INIT_Z["autile"] = 7

# 22. frosted grey "Aa" tile. v1-v5 started this at y95, almost flush under the sticky-note
# stack (which ends ~y120) -- compare.py's grid flagged x667-800,y114-171 as 0.98 OVER-filled
# (reference has 0.01 there): the reference has a visible GAP between the sticky stack and
# the Aa/Au/book cluster that v1-v5 never left room for. Re-measured off the crop:
# crop-local x140-320,y95-265 -> absolute x690-870,y145-315 (trimmed to the card's own right
# edge at 865).
GX, GY, GW, GH = 690, 145, 170, 170
PILE_BOX["aatile"] = (GX, GY, min(GW, 865 - GX), GH)
LABEL_BOX["aatile"] = (GX + 20, GY + 40, PILE_BOX["aatile"][2] - 40, GH - 80)
INIT_Z["aatile"] = 6

# 24. green "Approved" chip. Re-measured off the crop: crop-local x0-90,y270-300 ->
# absolute x550-640,y320-350 (v1-v5 had it 30px lower, sitting on top of the shortened
# ribbed rect instead of above it).
PX, PY, PW, PH = 550, 320, 90, 30
PILE_BOX["approved"] = (PX, PY, PW, PH)
LABEL_BOX["approved"] = (PX, PY, PW, PH)
INIT_Z["approved"] = 8

# 16. sticky-note stack. Re-measured off the crop: crop-local x40-110,y10-70 ->
# absolute x590-660,y60-120.
SX, SY, SW, SH = 590, 60, 65, 55
...
for k in draw_order:
    ...
    if k == "qcard":
        ...
        x, y, w, h = PILE_BOX[k]
        # core.PHOTOS['food'] carries a baked-in caption band ("STANDING STEADFAST FOR THE
        # ONES WHO NEED US MOST") in its own bottom ~15% -- a real caption on a real AQ photo,
        # not fabricated, but illegible/distracting clipped into a small pile tile. A plain
        # center/cover crop cannot hide it here: this tile (165x290, aspect 0.57) is TALLER
        # than the source photo (aspect ~0.8), so cover's height-driven scale shows the photo's
        # full height with no vertical crop margin at all -- confirmed by looking at v2.png,
        # where the caption band rendered in full across the tile's bottom third. Fix: an
        # explicit oversized background-size + top-anchored position, so the excess height
        # (including the caption) is pushed past the box's bottom edge and clipped away.
        parts.append(f'<div style="position:absolute;left:{x}px;top:{y}px;width:{w}px;height:{h}px;'
                     f'background:url({core.PHOTOS["food"]}) top center / 260px 460px no-repeat;'
                     f'border-radius:6px;box-shadow:0 8px 18px rgba(10,10,10,.25);z-index:{z}"></div>')
    elif k == "docard":
        x, y, w, h = PILE_BOX[k]
        parts.append(f'<div style="position:absolute;left:{x}px;top:{y}px;width:{w}px;height:{h}px;'
        ...
        x, y, w, h = PILE_BOX[k]
        # var(--d) (NeutralFace) is a display face meant to be set UPPERCASE (CLAUDE.md Sec9)
        # and its lowercase glyphs read as unicase caps -- "Aa" rendered as "AA" in v1/v2,
        # which defeats the whole point of a type-specimen tile (showing a mixed-case pair).
        # var(--e) (Eina, the body face) has a real lowercase 'a'.
        parts.append(f'<div style="position:absolute;left:{x}px;top:{y}px;width:{w}px;height:{h}px;'
                     ...
                     f'justify-content:center;z-index:{z}"><span style="font-family:var(--e);'
                     f'font-weight:800;font-size:60px;color:#FFFFFF">Aa</span></div>')
    elif k == "approved":
        x, y, w, h = PILE_BOX[k]
        ...
        x, y, w, h = PILE_BOX[k]
        # v3's 3 layers were staggered by only 2-5px, so the tan base note never actually
        # peeked past the dark-green one behind it (looking gate caught 2 visible layers,
        # not the audit's 3). Wider stagger, same footprint.
        parts.append(f'<div style="position:absolute;left:{x+16}px;top:{y+18}px;width:{w-14}px;'
                     f'height:{h-14}px;background:#D8CBB0;border-radius:8px;z-index:{z-2}"></div>')
        parts.append(f'<div style="position:absolute;left:{x+8}px;top:{y+9}px;width:{w-14}px;'
...
CONTAINMENT_IGNORE = PILE_PAIRS | {
    ...
    # the widened chair panel's top edge grazes the card's rounded bottom corner by a few px
    # -- it sits BEHIND the card (z=1 vs z=4) so it's invisible there anyway; only the strip
    # below the card (y565+) actually shows, matching the reference's own "behind the box"
    # bleed-panel construction.
    ("card_bg", "bleed_bmid_chair"),
}
for k in PILE_KEYS:
...
async def main():
    ...
    async with B.session():
        await B.render(html, f"{outdir}/v11.png", W, H, elements=els, color_pairs=color_pairs,
                        page_bg=CREAM, expect_hero=True, collision_ignore=frozenset(CONTAINMENT_IGNORE))
    print("done ->", f"{outdir}/v11.png")
