...
Run:  PYTHONIOENCODING=utf-8 python scratchpad/c1_v3.py

v2 fixes over v1 (found by the looking gate, see scratchpad/friction/c1.md):
  - CRITICAL: the body-copy paragraph was completely invisible. Cause: it had no explicit
    z-index (defaulted to `auto`/stacking level 0), while BLOCK_D's own background rect
    carried z-index:1 -- and an element with an explicit z-index ALWAYS paints above one at
    z-index:auto, regardless of DOM order. The opaque cream rectangle was silently painted
    OVER the entire paragraph. No static gate catches this (the colour itself is fine; it's
    pure stacking order) -- only Reading the PNG caught it. Fixed by giving every foreground
    text element an explicit z-index >= 10, consistently.
  - Tape-strip elements now use layout.rotated_bbox() for their declared collision box
    instead of the unrotated w/h, closing the "BBOX UNDER-REPORTS" advisory v1 printed.
  - Added a small department tag chip in the (very empty) cream block to balance it against
    the busy left/top blocks, per the "scale/add before it reads as a dead quadrant" rule.

v3 fixes over v2 (found by the looking gate):
  - The cream block had ~300px of dead, unintentional space between the end of the body
    paragraph and the footer (footer was pinned to a fixed offset from the block's BOTTOM
    edge rather than from the content above it) -- an accidental gap, not a rhythm choice.
    Footer now sits a fixed, deliberate 56px below the paragraph's own measured bottom.
  - The department tag chip had no shadow while every other object in the piece (tape,
    logo pill) carries the house hard-offset shadow -- gave it one for a consistent craft
    language.
"""
...
async def main():
    ...
    elements.append(("tape_showing", *lay.rotated_bbox(48, 672, t1_w, t1_h, -4)))

    t2_html, t2_w, t2_h = tape_strip("UP AGAIN", t2_m, 560, 726, 3, 58)
    parts.append(t2_html)
    elements.append(("tape_upagain", *lay.rotated_bbox(560, 726, t2_w, t2_h, 3)))

    t3_html, t3_w, t3_h = tape_strip("AND AGAIN", t3_m, 48, 800, -3, 50)
    parts.append(t3_html)
    elements.append(("tape_andagain", *lay.rotated_bbox(48, 800, t3_w, t3_h, -3)))
    ...
    # department tag chip -- fills the top of the otherwise-empty cream block and echoes
    # the eyebrow in block A, giving the piece a matching mark in every quadrant.
    tag_y = dy + 130   # clears the tape_upagain cluster, which bleeds into this block's top ~90px
    parts.append(f'<div style="position:absolute;left:{dx+52}px;top:{tag_y}px;background:{MINT};'
                 f'color:{TAPE_TEXT_ON_MINT};font-family:var(--m);font-weight:700;font-size:14px;'
                 f'letter-spacing:.12em;text-transform:uppercase;padding:8px 14px;border-radius:999px;'
                 f'border:3px solid {INK};box-shadow:{core.hard_shadow("sm")};z-index:10">welfare programme</div>')
    elements.append(("dept_tag", dx + 52, tag_y, 220, 36))

    body_x, body_y = dx + 52, tag_y + 60
    body_w = body_m["w"]
    # z-index:10 -- MUST be set explicitly. Left at `auto` in v1, this paragraph was drawn
    # entirely UNDER block D's own z-index:1 background rect and was 100% invisible on the
    # rendered PNG despite having a correct bbox and a correct (very legible) ink-on-cream
    # colour. No static gate catches an occlusion bug like this.
    ...
    # footer sits a fixed, deliberate gap BELOW THE PARAGRAPH'S OWN measured bottom, not
    # pinned to the block's bottom edge -- v2 pinned it to the edge and left ~150px of
    # accidental dead space between "again." and the handle.
    footer_y = body_y + body_m["h"] + again_m["h"] + 56
    ...
    with open("out/session10f/c1_v3.html", "w", encoding="utf-8") as f:
        f.write(html)
    async with B.session():
        await B.render(html, "out/session10f/c1_v3.png", W, H, elements=elements,
                       color_pairs=color_pairs, page_bg=INK, expect_hero=True)
    print("done -> out/session10f/c1_v3.png")
