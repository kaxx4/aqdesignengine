...
async def main():
    ...
    text_x, text_y, text_w = card_x + 90, card_y + 210, card_w - 180
    body = (f'you’re doing<br>'
            f'<span style="font-family:var(--s);font-style:italic">amazing</span>!<br>'
            f'let’s take shikshaq’s<br>reading circle to<br>the next level —<br>'
            f'soon these kids<br>will be reading<br>like pros!')
    ...
    # declared box corrected to the REAL drawn size reconcile_boxes reported (761x665,
    # not the 720x800 first guessed) — the false "note_text/fire collision" v2 first
    # printed was this stale bbox overlapping fire's declared box even though nothing
    # touches on screen; CLAUDE.md's own bug catalog calls this exact pattern out
    # ("stale bbox tuple hides/creates a false positive").
    elements.append(("note_text", text_x, text_y, 760, 665))
