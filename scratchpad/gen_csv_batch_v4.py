...
async def piece_one():
    ...
    # Digits have no descenders, so ink_h reserves empty space under the numeral
    # and flowing off it opened a visible hole. Flow off the LAYOUT box plus a
    # real gap, and lift the stickers so they still clear the painted glyphs.
    ty = hero_y + hero_h + 34
    ...
    sx = M - 12 + hero_w + 26
    parts.append(keyline_sticker(sx, hero_y + 30, 168, LEMON, "ALL<br>WELFARE", rot=-9))
    els.append(("stk1", *lay.rotated_bbox(sx, hero_y + 30, 168, 168, -9)))
    parts.append(keyline_sticker(sx + 6, hero_y + 214, 150, PINK,
                                 f"{OBJ['Workshop']}<br>WORK&shy;SHOPS", rot=7, fs=16))
    els.append(("stk2", *lay.rotated_bbox(sx + 6, hero_y + 214, 150, 150, 7)))
    ...
    # v3 left this floating with nothing under it, so it read as a smudge rather
    # than tape. Tape has to be holding something down.
    parts.append(tex.tape(sx + 92, hero_y + 6, 120, 38, rot=-14, z=30, color="#E4D7A8"))
