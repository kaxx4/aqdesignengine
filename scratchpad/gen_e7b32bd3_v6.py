...
clipped("bleed_bl_pages", 0, 390, 156, 125, "0% 0%, 100% 32%, 100% 100%, 0% 100%",
...
# rows 4-8), but compare.py's grid showed the reference's right edge (col 9/9) is EMPTY in
# that whole middle band -- it only has content at the very top (the tulip corner, rows 0-2)
# and very bottom (the swing tag, rows 9-10). Narrowed so it stops short of the true edge;
# only the corner elements (tulip, tag) actually touch x=1200.
clipped("bleed_rmid_flowers", 1002, 302, 148, 257, "35% 0%, 100% 0%, 100% 100%, 0% 100%",
...
async def main():
    ...
    async with B.session():
        await B.render(html, f"{outdir}/v6.png", W, H, elements=els, color_pairs=color_pairs,
                        page_bg=CREAM, expect_hero=True, collision_ignore=frozenset(CONTAINMENT_IGNORE))
    print("done ->", f"{outdir}/v6.png")
