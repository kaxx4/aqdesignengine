...
async def main():
    ...
    pile_centers = {
        ...
        "dribbble":  (0.680, 0.455),
        "behance":   (0.545, 0.560),
        "facebook":  (0.585, 0.690),
    }
    ...
    # v12->v13: v12 put BEHANCE (z=14) IN FRONT of INSTAGRAM (z=12), which cut
    # "INSTAGRAM" down to "INSTAGR" — a real miss that resolve_label_z did NOT
    # name (it reported 'dribbble' unresolved instead: raising dribbble clear
    # of behance re-buried instagram, a 3-way cycle a single pairwise re-stack
    # can't fully solve). Two fixes: (a) reorder the z-seed to match the
    # reference (BEHANCE tucks BEHIND both neighbours there, not in front of
    # either), (b) shrink a PILL's protected "label" box from the whole
    # capsule to the inner region that actually holds the word — the rounded
    # end-caps are padding, and treating the full capsule as sacred turns an
    # end-cap graze into the same severity as a word clipped in half.
    z_seed = {"hand": 10, "linkedin": 11, "behance": 12, "instagram": 13,
              "dribbble": 14, "facebook": 15, "heart": 18, "tongue": 20}
    label_boxes = {}
    for key, (x, y, w, h, rot) in pile_boxes.items():
        if key in sticker_specs:
            ...
            inset = PAD * 0.75   # PAD is the pill's own text-padding constant
            label_boxes[key] = (x+inset, y, w-2*inset, h)
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v14.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v14.png")
