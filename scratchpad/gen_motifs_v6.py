...
async def piece_a():
    ...
    # NO hand-typed coordinates. v1-v5 placed these ten badges by eye and every
    # single fix traded one collision for another: buried DOG FEEDS, then buried the
    # word "UP.", then two badges on the body copy. layout.scatter_solve does the
    # search properly against the real constraints.
    SW_ = [
        ("scallop", LEMON,  196, ["ALL", "WELFARE"],      -11),
        ("burst",   PINK,   196, ["291", "WORK", "SHOPS"],  9),
        ("capsule", MINT,   244, ["PATHER SATHI"],          7),
        ("gear",    GRAPE,  158, ["EK", "TARA"],          -14),
        ("tag",     TOMATO, 214, ["DOG FEEDS"],             6),
        ("blob",    SKY,    166, ["SUNDAR", "BANS"],       -7),
        ("arch",    LEMON,  164, ["26", "DRIVES"],         -8),
        ("shield",  MINT,   152, ["DISHA"],                11),
        ("blob",    PINK,   144, ["SINCE", "2021"],        -9),
        ("scallop", SKY,    150, ["KOLKATA"],               8),
    ]
    flat = [(i, t) for i, (_, _, _, ls, _) in enumerate(SW_) for t in ls]
    ...
    HEAD_LINES = ["WE JUST", "KEEP", "SHOWING UP."]
    # SOLVE the size so the longest line actually fits the column. v4 set 152px and
    # "SHOWING UP." silently wrapped, so the headline rendered as FOUR lines while the
    # gate was told about three — and the badges placed against those three boxes
    # buried the word "UP." completely. Legibility is the one gate that cannot be
    # traded away, and it failed because a planned line count was never verified.
    probe = await B.measure_text([{"text": t, "font": "d", "size": 152, "weight": 900}
                                  for t in HEAD_LINES])
    widest = max(p["glyph_w"] for p in probe)
    HFS = int(min(152, 152 * (W - 2 * M - 8) / widest))
    m = await B.measure_text(
        [{"text": "<br>".join(HEAD_LINES), "font": "d", "size": HFS,
          "weight": 900, "line_height": .86, "max_width": W - 2 * M}]
        + [{"text": t, "font": "d", "size": HFS, "weight": 900} for t in HEAD_LINES])
    head_h = m[0]["h"]
    if m[0]["lines"] != len(HEAD_LINES):
        print(f"    ! headline rendered {m[0]['lines']} lines, not the "
              f"{len(HEAD_LINES)} the badge placement assumes")
    else:
        print(f"    piece A: headline solved to {HFS}px, {m[0]['lines']} lines as planned")
    # A three-line headline's block bbox is ~60% empty air, so declaring it as ONE
    # box makes collision_check fail on badges that sit in the gaps beside the short
    # lines, and makes occlusion_check report 0% visible for badges that are plainly
    # in the clear. Declare the LINES instead: "KEEP" is 420px wide, not 952.
    line_h = head_h / len(HEAD_LINES)
    line_boxes = [(f"line{i}", M, hy_ + i * line_h, m[1 + i]["glyph_w"], line_h)
    ...
    # v2 buried two badges under the headline: occlusion_check measured DOG FEEDS at
    # 0% visible and KOLKATA at 21%. Depth needs SOME of the badge showing, so these
    # move to the headline's edge where being partly behind reads as intended.
    # Solve the placement. Protected: the three headline lines and the body copy —
    # coverable a little (that is the motif) but never buried. Keep-out: the logo,
    # the footer and the stat strip, which nothing may touch at all.
    body_y = hy + head_h + 66
    protect = [lb[1:] for lb in line_boxes] + [(M, body_y, 790, 96)]
    keep_out = [(0, 0, W, 132), (0, H - 300, W, 300)]
    foot = [(lab, *lay.rotated_bbox(0, 0, size, size, rot)[2:])
            for (lab, size, rot) in
            [(f"{k}-{i}", sz, rt) for i, (k, _c, sz, _l, rt) in enumerate(SW_)]]
    spots, unplaced = lay.scatter_solve(
        foot, W, H, protect=protect, keep_out=keep_out,
        zones=[(M - 40, hy - 160, W - 2 * M + 80, head_h + 200),
               (M - 30, hy + head_h + 150, W - 2 * M + 60, 440)],
        max_pair_overlap=0.17, max_protect_cover=0.26, seed=23)
    if unplaced:
        print(f"    ! scatter_solve could not place {unplaced} — pile is too dense")
    pos = {lab: (x, y) for lab, x, y in spots}

    for i, (kind, fill, size, lines, rot) in enumerate(SW_):
        key = f"{kind}-{i}"
        if key not in pos:
            continue
        bx, by_ = pos[key]
        rb = lay.rotated_bbox(0, 0, size, size, rot)
        x = bx - rb[0]
        y = by_ - rb[1]
        # Depth comes from badge-on-badge overlap, NOT from putting badges behind
        # the type. A badge behind a headline loses its own label, which is what
        # buried DOG FEEDS, KOLKATA and ALL WELFARE across v1-v6. In the reference
        # every badge is in FRONT; the headline reads because the badges land in its
        # gaps, and the pile reads as a pile because the badges overlap each other.
        z = 28 + (i % 4)
        P.append(place(badge(kind, fill, size, lines, wmap[i], rot), int(x), int(y), size, z))
        els.append((key, bx, by_, rb[2], rb[3]))
        ZED.append((key, (bx, by_, rb[2], rb[3]), z))

    P.append(f'<div style="position:absolute;top:{body_y}px;left:{M}px;width:790px;'
    ...
    for lab, frac in lay.occlusion_check(ZED):
        print(f"    ! {lab} only {frac*100:.0f}% visible behind what sits in front")
...
async def piece_b():
    ...
    PAD, DOT, GAP, RH, RGAP, BORD = 28, 20, 20, 74, 16, 3
    ...
    while y < H - 80:
        ...
        while x < W + 40:
            ...
            # page() sets `* { box-sizing: border-box }`, so the 3px border on each
            # side comes OUT of the content width. v3 sized every pill to exactly its
            # content and the gate caught all 28 of them clipping by 5-6px — a defect
            # small enough to miss by eye but real in every single pill.
            pw = PAD + DOT + 13 + tw + PAD + 2 * BORD + 2
            c = hues[idx % len(hues)]
            P.append(
                f'<div style="position:absolute;left:{x}px;top:{y}px;width:{pw}px;height:{RH}px;'
                f'background:{PAPER};border:{BORD}px solid {INK};border-radius:999px;'
...
# ════════════════════════════════════════════════════════════════════════════
async def piece_c():
    """The lower block is SOLVED to the room left under the photo, not guessed.

    v2 set the headline at 94px, it wrapped to four lines with "IS" orphaned on its
    own, and the body copy landed on top of the footer. The available height was
    knowable the whole time: footer top, minus photo bottom, minus the gaps and the
    body. layout.fit_block does that arithmetic.
    """
    lines = ["FIELD", "PROGRAM"]
    lm = await B.measure_text([{"text": t, "font": "d", "size": REF, "weight": 800}
                               for t in lines])

    PX, PY, PW, PH = M, 188, W - 2 * M, 628
    FOOT, GAP, BODY_H = 124, 46, 46
    room = (H - FOOT) - (PY + PH) - GAP - BODY_H - 30
    HEAD_LINES = 3
    HFS = int(lay.fit_block(room, HEAD_LINES, line_height=0.9, max_size=96, min_size=52))
    HEAD = "THE CLASSROOM<br>IS A BOAT<br>RIDE AWAY."
    m = await B.measure_text([{"text": HEAD, "font": "d", "size": HFS, "weight": 900,
                               "line_height": .9, "max_width": W - 2 * M}])
    hh, hlines = m[0]["h"], m[0]["lines"]
    if hlines != HEAD_LINES:
        print(f"    ! headline wrapped to {hlines} lines, not the {HEAD_LINES} planned")

    els, P = [], []
    P.append(f'<div style="position:absolute;inset:0;background:{CREAM};'
             f'{tex.paper_fibre(0.05)}"></div>')
    P.append(f'<div style="position:absolute;left:{PX}px;top:{PY}px;width:{PW}px;'
             f'height:{PH}px;border:5px solid {INK};'
             f'border-radius:{core.RADII["outer"]}px;overflow:hidden;'
             f'box-shadow:{core.hard_shadow("xl")};z-index:6">'
             + tex.duotone(core.PHOTOS["edu"], shadow=INK, highlight=MINT,
                           size_css="width:100%;height:100%", screen=True, screen_size=6)
             + '</div>')
    els.append(("photo", PX, PY, PW, PH))

    P.append(B.logo(dark=True))
    P.append(f'<span style="position:absolute;top:62px;right:{M}px;font-family:var(--m);'
             f'font-weight:700;font-size:15px;letter-spacing:.14em;'
             f'color:{core.on_cream(MINT,15)};z-index:60">EDUCATION &middot; SUNDARBANS</span>')

    ssz = 196
    sx, sy = W - M - 166, PY + PH - 168
    P.append(place(badge("burst", LEMON, ssz, lines, [x["w"] for x in lm], rot=-12),
                   sx, sy, ssz, 22))
    els.append(("stk", *lay.rotated_bbox(sx, sy, ssz, ssz, -12)))

    ty = PY + PH + GAP
    P.append(f'<div class="measure" data-tag="title" style="position:absolute;top:{ty}px;'
             f'left:{M}px;width:{W-2*M}px;font-family:var(--d);font-weight:900;'
             f'font-size:{HFS}px;line-height:.9;letter-spacing:-.025em;color:{INK};'
             f'z-index:10">{HEAD}</div>')
    els.append(("title", M, ty, W - 2 * M, hh))
    P.append(f'<div style="position:absolute;top:{ty+hh+26}px;left:{M}px;width:{W-2*M}px;'
             f'font-family:var(--e);font-size:26px;line-height:1.4;color:{INK};'
             f'opacity:.78;z-index:10">ten sundarbans relief runs logged. '
             f'the delta has no bus route.</div>')
    els.append(("body", M, ty + hh + 26, W - 2 * M, BODY_H))
    print(f"    piece C: {room}px of room -> headline solved to {HFS}px, {hlines} lines")
    P.append(footer(CREAM, "SUNDARBANS RELIEF \u00b7 10 RUNS"))
    return (B.page(W, H, CREAM, "".join(P), grain=False),
            f"{OUT}/C_duotone_photo.png", els, CREAM)
