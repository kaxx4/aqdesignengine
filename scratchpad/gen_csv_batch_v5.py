...
async def piece_one():
    ...
    The hero SIZE is solved for, not picked. We measure the numeral once, then
    choose the font-size that leaves exactly enough room for the sticker column
    beside it. v1-v4 picked a size by eye and then discovered (three times) that
    the painted glyphs were wider than the box the gate was checking.
    """
    W, H = core.SIZES["feed"]; M = 64
    STK, ROT, GAP = 168, -9, 28
    stk_w = lay.rotated_bbox(0, 0, STK, STK, ROT)[2]
    budget = (W - M) - (M - 12) - GAP - stk_w        # px the numeral may paint into

    probe = await B.measure_text([
        {"text": str(PATHER), "font": "d", "size": 400, "weight": 900,
         "line_height": 0.78, "letter_spacing": "-.045em"}])
    HERO_FS = int(400 * budget / probe[0]["glyph_w"])          # solve for the fit
    ...
    m = await B.measure_text([
        ...
        {"text": "CAME BACK.", "font": "d", "size": 92, "weight": 900},
    ])
    hero_h, gw, gh = m[0]["h"], m[0]["glyph_w"], m[0]["glyph_h"]
    ...
    hero_y = 214
    hx = M - 12
    parts.append(f'<div style="position:absolute;top:{hero_y}px;left:{hx}px;font-family:var(--d);'
    ...
    # the bbox is the PAINTED box, centred in the line box - not either CSS box
    paint_top = hero_y + (hero_h - gh) / 2
    els.append(("hero", hx, paint_top, gw, gh))

    ty = int(paint_top + gh + 30)
    ...
    sx = int(hx + gw + GAP)
    parts.append(keyline_sticker(sx, hero_y + 26, STK, LEMON, "ALL<br>WELFARE", rot=ROT))
    els.append(("stk1", *lay.rotated_bbox(sx, hero_y + 26, STK, STK, ROT)))
    parts.append(keyline_sticker(sx + 8, hero_y + 218, 150, PINK,
                                 f"{OBJ['Workshop']}<br>WORK&shy;SHOPS", rot=7, fs=16))
    els.append(("stk2", *lay.rotated_bbox(sx + 8, hero_y + 218, 150, 150, 7)))
    parts.append(doodle("sparkle", M + title_w + 40, ty + 34, 96, TOMATO, rot=-14, z=13))
    els.append(("dd1", M + title_w + 40, ty + 34, 96, 96))
    # tape has to be holding something down - here, the corner of the lemon sticker
    parts.append(tex.tape(sx + 86, hero_y - 2, 118, 38, rot=-16, z=30, color="#E4D7A8"))
    ...
    parts.append(f'<div style="position:absolute;left:{M}px;top:{sy}px;width:{W-2*M}px;height:176px;'
                 ...
                 f'<div style="font-family:var(--e);font-size:25px;margin-top:10px;'
    ...
    print(f"    piece1: hero solved to {HERO_FS}px (paints {gw}x{gh} into a {budget}px budget)")
...
async def piece_two():
    ...
    ry, rh, gap = by + 78, 88, 20
    track = W - M - (M + GUT) - 96                 # room left for the count at the end
    for i, (lab, n) in enumerate(order):
        y = ry + i * (rh + gap)
        bx = M + GUT
        bw = max(46, int(track * n / top))
        c = hues[i % len(hues)]
        parts.append(f'<div style="position:absolute;left:{M}px;top:{y+rh/2-12}px;width:{GUT-20}px;'
        ...
        parts.append(f'<div style="position:absolute;left:{bx+bw+20}px;top:{y+18}px;font-family:var(--d);'
                     f'font-weight:900;font-size:50px;line-height:1;color:{CREAM};z-index:9">{n}</div>')
