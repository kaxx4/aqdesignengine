...
async def main():
    ...
    # v6->v9: v6/v7/v8 all plateaued around score 0.17-0.19 with the SAME two non-blocking
    # region critiques every time regardless of random seed (row6/col6 under-filled, row9/col3
    # under-filled) — a single scatter_solve call for all 3 stickers has no notion of MY OWN
    # step-1 inventory, which explicitly records hand=upper-left, tongue=centre, heart=far
    # RIGHT of the pile. The solver kept landing "heart" on the LEFT instead (nothing forces a
    # left-to-right reading), which is exactly why the right-hand region reads empty and the
    # left is over-dense. Still using scatter_solve (never hand-typed coordinates) but calling
    # it three times, once per sticker, each biased into a THIRD of the zone matching the
    # written inventory, with the earlier stickers as keep_out so they cannot collide.
    zx, zy, zw, zh = zone
    thirds = {
        "hand":   (zx, zy, zw * 0.40, zh),
        "tongue": (zx + zw * 0.28, zy, zw * 0.44, zh),
        "heart":  (zx + zw * 0.60, zy, zw * 0.40, zh),
    }
    stk_rw = {l: (rw, rh) for l, rw, rh in sticker_items}
    stk_placements, stk_unplaced = [], []
    placed_sticker_boxes = []
    for label in ["hand", "tongue", "heart"]:
        item = [(l, rw, rh) for l, rw, rh in sticker_items if l == label]
        # earlier stickers are `protect`, not `keep_out` — a little sticker-on-sticker
        # overlap is still the reference's own look, just no longer left to chance which
        # SIDE each sticker ends up on.
        p, u = lay.scatter_solve(
            item, W, H, protect=placed_sticker_boxes, max_protect_cover=0.22,
            keep_out=[wm_block, cross_box, foot_box], zones=[thirds[label]], margin=10,
            max_pair_overlap=0.22, seed=23, tries=2500)
        stk_placements += p; stk_unplaced += u
        for lab, rx, ry in p:
            placed_sticker_boxes.append((rx, ry, *stk_rw[lab]))
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v9.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v9.png")
