...
67805068493b4522f5c3944723bee7d2.jpg), v4.
...
async def build():
    ...
    ring = [
        ...
        ("shieldYellow", S.shield(),                                          A[2], 0.83, 0.285, 0.20,   8,  18),
        ...
        ("flowerPink",   S.scallop(lobes=9, r=44),                             A[0], 0.845,0.405, 0.185,  0,  20),
        ("animalPink",   S.capsule(w=100, h=60),                              A[0], 0.735,0.46,  0.20,   16,  22),
        ("spiralBlue",   None,                                                A[4], 0.09, 0.45,  0.145,  0,  20),  # doodle
        ("spiralYellow", None,                                                A[2], 0.185,0.515, 0.115, 15,  22),  # doodle
        ("flowerGreen2", S.blob(seed=9, lobes=8, wobble=0.32),                 A[1], 0.87, 0.545, 0.135, 10,  24),
        ("flowerBrown",  S.scallop(lobes=8, r=44),                            BROWN_DK,0.775,0.645,0.135,-6,  20),
        ("kiteBlueLt",   S.starburst(points=4, R=50, r=16),                    A[4], 0.665,0.695, 0.115,  6,  22),
        ("bearBrown",    S.blob(seed=11, lobes=8, wobble=0.26),               BROWN_MD,0.30, 0.605, 0.29,  -10, 18),
        ("birdCream",    S.blob(seed=13, lobes=6, wobble=0.2),                CREAMY,0.53, 0.59,  0.115,  8,  22),
    ]
    ...
    x, y = F(0.35, 0.42);   els_html.append(word("to", x, y, 30, rot=-4, label="w_to"))
    x, y = F(0.375, 0.41);  els_html.append(word("show", x, y, 92, rot=-2, italic=True, label="w_show"))
    x, y = F(0.28, 0.475);  els_html.append(word("up", x, y, 62, rot=3, label="w_up"))
    x, y = F(0.63, 0.505);  els_html.append(word("for", x, y, 30, rot=-6, label="w_for"))
    x, y = F(0.505, 0.525); els_html.append(word("someone", x, y, 60, rot=0, label="w_someone"))
    x, y = F(0.795, 0.485); els_html.append(word("else?", x, y, 46, rot=4, label="w_else"))

    # ---------------- wordmark, two lines + thumbsup, echoing the reference's PLAYBOOK block
    wx, wy = F(0.335, 0.715)
    tw = 0.33 * W
    els_html.append(at(wx, wy, tw, 200,
        f'<div style="font-family:var(--d);font-weight:900;font-size:58px;line-height:1.02;'
    ...
    bleed_tags = ("shieldYellow", "starRedBig")
    ...
    out_png = f"out/versions/{slug}/v4.png"
