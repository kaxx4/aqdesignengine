...
# ---------------------------------------------------------------------------
def sign_leg_path(w, h, top_in=0.14, notch_y=0.85, foot_in=0.26):
...
async def main():
    ...
    # v2->v3: bleed the leg to the canvas edge (leg_w widened to reach x=1080) —
    # the reference's black shape runs off the RIGHT EDGE of the phone screen
    # itself, v2 left a visible green margin there that compare.py's region grid
    # flagged as OVER-filled one column in and under-filled at the true edge.
    # Shortened feet (notch_y 0.78->0.85) to match the reference's shorter legs.
    leg_x, leg_y, leg_w, leg_h = 560, 232, 520, 720
    ...
    # v2->v3: narrowed back to the SIGN's own width (v2 spanned the full
    # sign+leg group, wider than the reference's promo block proportion)
    col_x, col_w = sign_x, sign_w + 270
    ...
    out_png = os.path.join(out_dir, "B_v3.png")
