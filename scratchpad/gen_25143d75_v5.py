...
async def main():
    ...
    zone = (int(W*0.08), int(H*0.30), int(W*0.80), int(H*0.53))
    dead_zone_keepout = (int(W*0.68), int(H*0.36), int(W*0.16), int(H*0.10))  # row5/col7 only
    bottom_bias = (int(W*0.08), int(H*0.52), int(W*0.80), int(H*0.31))
    ...
    pill_placements, pill_unplaced = lay.scatter_solve(
        pill_items, W, H, protect=sticker_boxes_for_protect, max_protect_cover=0.45,
        keep_out=keep_out, zones=[zone, bottom_bias],
        margin=10, max_pair_overlap=0.14, seed=23, tries=3000)
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v5.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v5.png")
