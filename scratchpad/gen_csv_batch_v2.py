...
async def piece_one():
    ...
    hero_w, hero_h = m[0]["w"], m[0]["h"]          # REAL extent, not a guess
    title_w, title_h = m[1]["w"], m[1]["h"]
    ...
    els.append(("hero", M - 12, hero_y, hero_w, hero_h))     # measured
    ...
    els.append(("title", M, ty, title_w, title_h))
    ...
    sy = ty + title_h + 34
    parts.append(f'<div style="position:absolute;left:{M}px;top:{sy}px;width:{W-2*M}px;height:152px;'
                 ...
                 f'</div></div>')
    els.append(("slab", M, sy, W - 2 * M, 152))
...
async def piece_two():
    ...
    m = await B.measure_text(
        ...
        + [{"text": "EVERY<br>SINGLE<br>ONE.", "font": "d", "size": 124, "weight": 900,
            "line_height": 0.86, "max_width": 900}])
    ...
    ty = 160
    parts.append(f'<div class="measure" data-tag="title" style="position:absolute;top:{ty}px;left:{M}px;'
                 f'width:900px;font-family:var(--d);font-weight:900;font-size:124px;line-height:.86;'
                 f'letter-spacing:-.03em;color:{CREAM};z-index:6">EVERY<br>SINGLE<br>ONE.</div>')
    els.append(("title", M, ty, 900, title_h))
    ...
    parts.append(f'<div style="position:absolute;top:{ty+8}px;right:{M}px;font-family:var(--d);'
                 f'font-weight:900;font-size:232px;line-height:.82;letter-spacing:-.04em;'
                 f'color:transparent;-webkit-text-stroke:3px {MINT};z-index:5">{len(ROWS)}</div>')
    parts.append(f'<div style="position:absolute;top:{ty+208}px;right:{M+6}px;font-family:var(--m);'
