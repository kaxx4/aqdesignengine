...
PAPER = "#FFFFFF"                    # card fill -- deliberately distinct from CREAM (>18 delta)
                                      # so same_as_bg_scan doesn't flag a "same as page bg" card;
                                      # the reference's own card differs from its field mainly by
                                      # shadow, but that cue isn't colour-measurable, so PAPER white
                                      # stands in ("hero is a pile on a PAPER ground" per the brief).
INK = core.INK
GREY = "#6B6B66"
...
TAG_X, TAG_Y, TAG_W, TAG_H = 1012, 470, 180, 150
parts.append(
    f'<div style="position:absolute;left:{TAG_X}px;top:{TAG_Y}px;width:{TAG_W}px;height:{TAG_H}px;'
    f'transform:rotate(-9deg);background:{A[1]};border-radius:10px;'
    f'box-shadow:3px 4px 0 {INK};z-index:2"></div>'
)
E("bleed_br_tag_bg", TAG_X - 6, TAG_Y - 4, TAG_W + 16, TAG_H + 16)
parts.append(
    f'<div style="position:absolute;left:{TAG_X+16}px;top:{TAG_Y+30}px;width:{TAG_W-30}px;'
    f'transform:rotate(-9deg);font-family:var(--d);font-weight:900;font-size:26px;line-height:1.0;'
    f'color:{INK};text-transform:uppercase;z-index:3">Thank<br>you</div>'
)
E("bleed_br_tag_text", TAG_X + 10, TAG_Y + 20, TAG_W - 20, 80)

# ---------------------------------------------------------------- 11. central card ("paper ground")
parts.append(
    f'<div style="position:absolute;left:{CARD_X}px;top:{CARD_Y}px;width:{CARD_W}px;height:{CARD_H}px;'
    f'border-radius:{CARD_R}px;background:{PAPER};box-shadow:0 18px 40px rgba(10,10,10,.14);'
    f'z-index:4"></div>'
)
...
step("Production", "02", 452, 170)
E("step_production", 452, 170, 155, 22)
step("Review", "03", 372, 380)
E("step_review", 372, 380, 115, 22)
step("Delivery", "04", 718, 380)
E("step_delivery", 718, 380, 130, 22)
...
# 26a/b. .jpg / .mp4 file tiles
JX, JY, JW, JH = 770, 415, 68, 88
PILE_BOX["jpgtile"] = (JX, JY, JW, JH)
LABEL_BOX["jpgtile"] = (JX, JY + JH - 26, JW, 26)
INIT_Z["jpgtile"] = 11

MX, MY, MW, MH = 795, 448, 68, 88
