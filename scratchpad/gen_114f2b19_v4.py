...
def progress_bar():
    ...
    html = (f'<div style="position:absolute;left:{track_x}px;top:{track_y}px;width:{track_w}px;'
            f'height:{track_h}px;background:#AFAFAF;border-radius:999px;z-index:5"></div>'
...
async def main():
    ...
    # star — v1 put nearly the WHOLE star behind the note (only a jagged black
    # sliver showed, LOOKING-GATE fail: "present" but not the right SIZE/
    # RELATIONSHIP — RECREATION_PROTOCOL's three questions). Reference has the star
    # mostly clear of the note, ABOVE its top edge, with only its bottom-right tip
    # tucked behind the corner. Moved up (y 300->205) and shrunk slightly so its
    # footprint clears the card's top edge (card top ~= 340 before rotation).
    star_x, star_y, star_s = 40, 205, 190
    ...
    # visually hangs FROM it). v2's compare.report against the crop showed an
    # OVER/UNDER-filled PAIR at row2/col5 (0.65 vs ref 0.23) and row3/col5 (0.31 vs
    # ref 0.69) — the RECREATION_PROTOCOL "wrong shape, not a missing element" tell
    # (adjacent over+under cells, balanced totals). The clip's mass was concentrated
    # too high; reference has it lower, right at the note's top edge. Shifted down.
    clip_x, clip_y, clip_w, clip_h = 390, 250, 300, 220
    ...
    card_html = (f'<div style="position:absolute;left:{card_x}px;top:{card_y}px;width:{card_w}px;'
                ...
                f'background:#FFFFFF;border-radius:18px;border:2px solid {INK};'
    ...
    # v1 set this at 44px and it filled only the top ~15% of the card, leaving a huge
    # dead-white gap in the middle (RECREATION_PROTOCOL's "right SIZE" question, not
    # just "present") — the reference's copy runs roughly 1/3 to 2/3 down the note.
    # Bumped the size and forced shorter explicit line breaks (mirroring the
    # reference's own short handwritten-note lines) so it occupies a comparable band.
    # v3's compare.report showed a further UNDER-filled cell at row6/col6 (ref 0.37,
    # mine 0.00) — the reference's paragraph runs wider (further right) than my
    # forced short line-breaks did. Widened the box and let more of it wrap
    # naturally instead of hand-breaking every line.
    text_x, text_y, text_w = card_x + 90, card_y + 210, card_w - 130
    body = (f'you’re doing '
            f'<span style="font-family:var(--s);font-style:italic">amazing</span>! '
            f'let’s take shikshaq’s reading circle to the next level — soon these kids '
            f'will be reading like pros!')
    text_html = (f'<div style="position:absolute;left:{card_x}px;top:{card_y}px;width:{card_w}px;'
                ...
                f'<div style="position:absolute;left:90px;top:210px;width:{text_w}px;'
                f'font-family:var(--e);font-weight:600;font-size:58px;line-height:1.32;'
                f'color:{INK}">{body}</div></div>')
    parts.append(text_html)
    # declared box widened to match; height re-measured after the wrap change (see
    # render output — reconcile_boxes reports the real drawn size every run, so this
    # gets corrected from that rather than guessed).
    elements.append(("note_text", text_x, text_y, 810, 700))
    ...
    # v3's compare.report showed UNDER-filled cells at row9/col6 (ref 0.92, mine 0.54)
    # and row10/col7 (ref 0.44, mine 0.00) — the flame's mass and reach were smaller
    # than the reference's. Enlarged and dropped it a bit lower off the card edge.
    fire_x, fire_y, fire_w, fire_h = 545, 1180, 360, 530
    ...
    text_pairs = [("note_text", INK, "#FFFFFF", 58, False)]
    ...
    # note_text/fire: a genuine BBOX overlap (25px) but not a visual one — the text
    # block's declared rect is left-aligned, ragged-right prose, and the 25px lives
    # entirely in the empty right margin of that rect, well clear of the fire icon's
    # actual pixels (confirmed by looking at the render). Declared here rather than
    # padded away, since padding it away would silently make the box wrong again.
    collision_ignore = {frozenset({"star", "note_card"}), frozenset({"clip", "note_card"}),
                        frozenset({"fire", "note_card"}), frozenset({"note_text", "fire"})}

    out_png = f"{outdir}/v4.png"
