...
async def main():
    ...
    # v10->v11 REFINEMENT: the first pass (fractions read off the 5%-grid
    # overlay) scored WORSE than v6 (0.225 vs 0.169) with compare.py flagging
    # REGION UNDER-filled at row6-7/col3 and row7/col7 — real gaps, not noise.
    # Re-measured against a finer 2%-grid crop (scratchpad/_ref_grid_fine.png)
    # and found the first pass had put "tongue" too far right (0.415 vs a
    # remeasured 0.37) and "dribbble"/"behance" too far left/up, leaving a
    # true negative-space gap between the hand/linkedin cluster and the
    # tongue/behance/dribbble cluster that the reference does not have — the
    # reference's objects interlock with almost no gap at all in that band.
    # Values below are the finer-grid remeasurement.
    pile_centers = {
        # stickers
        "hand":      (0.220, 0.475),
        "tongue":    (0.370, 0.650),
        ...
        "linkedin":  (0.223, 0.748),
        "instagram": (0.470, 0.430),
        "dribbble":  (0.665, 0.480),
        "behance":   (0.563, 0.580),
        "facebook":  (0.708, 0.690),
    }
    ...
    async with B.session():
        issues = await B.render(html, f"{OUTV}/v11.png", W, H, elements=elements,
                                color_pairs=color_pairs, page_bg=INK,
                                collision_ignore=collision_ignore)
    print("render issues:", issues)
    print("done -> ", f"{OUTV}/v11.png")
