...
CARD_D = dict(x=496, y=730, w=500, h=460)   # photo 2 -- food distribution photo
                                             # (shorter than v2: at box-width 500 the cover-fit
                                             # scale is FIXED at 625px regardless of box height,
                                             # so box height is the only lever that buys more
                                             # natural crop -- see the photo-crop note below)
...
async def build():
    ...
    # one hero-shine per piece (CLAUDE.md sec.9 craft layer) -- a soft diagonal
    # highlight sweep across the hero card only, low enough opacity to sit BEHIND
    # the ink numeral without touching its contrast (checked below, not assumed).
    # Diagonal via the GRADIENT ANGLE, not a rotated/oversized element -- a rotated
    # child sized past its own card and relying on the card's overflow:hidden to
    # crop it re-triggers the exact reconcile.measure_dom false-positive documented
    # above for the photo crops (CLIPPED/OFF-CANVAS on an ancestor-clipped child it
    # can't see is intentional). A same-size gradient layer gets the same sweep with
    # nothing for that check to trip on.
    a_shine = (f'<div style="position:absolute;inset:0;background:linear-gradient(125deg,'
               f'rgba(255,255,255,.22) 0%,rgba(255,255,255,0) 38%);pointer-events:none"></div>')
    a_inner = (
        a_shine +
        f'<div style="position:absolute;left:32px;top:26px;font-family:var(--m);font-weight:700;'
        f'font-size:15px;letter-spacing:.12em;color:{TXT_ON_GRAPE};opacity:.72">COUNTED, NOT GUESSED</div>'
    ...
    # Both source photos are finished IG posts, not raw photography -- each carries
    # its own baked-in caption line + small logo watermark + star-garland overlay
    # (core.PHOTOS has no raw alternative; this is the real-assets-only rule's actual
    # shape here). v2 cropped the education photo clear but left the food photo's
    # "STANDING STEADFAST FOR THE..." caption bleeding into the last ~90px of the
    # card, clipped mid-sentence -- a stray, half-legible line of text that reads as
    # a mistake, not craft.
    #
    # FIRST FIX TRIED, REVERTED: oversizing the <img> (height:118% / transform:
    # scale()) to crop more, still inside overflow:hidden. Visually fine, but
    # reconcile.measure_dom has no concept of an ancestor's overflow:hidden -- it
    # flags ANY child whose own bounding rect exceeds its parent's as CLIPPED /
    # OFF-CANVAS, so a routine "zoom the photo to crop out a caption" move reads as
    # a hard failure in render()'s returned issues even though nothing is visibly
    # wrong. Real gate gap, kept as plain object-fit:cover instead: at a fixed box
    # WIDTH the cover-fit scale is fixed (1350*500/1080=625px tall here) regardless
    # of box height, so box height is the only free lever for MORE natural crop --
    # cardD was shortened (580->460px) instead of the image being oversized.
    c_inner = (f'<img src="{core.PHOTOS["edu"]}" style="width:100%;height:100%;object-fit:cover;'
               ...
               f'<div style="position:absolute;inset:0;background:linear-gradient(180deg,rgba(10,10,10,0) 55%,rgba(10,10,10,.8) 100%)"></div>')
    d_inner = (f'<img src="{core.PHOTOS["food"]}" style="width:100%;height:100%;object-fit:cover;'
               f'object-position:50% 0%;filter:saturate(.9) contrast(1.08)">'
               f'<div style="position:absolute;inset:0;background:linear-gradient(180deg,rgba(10,10,10,0) 55%,rgba(10,10,10,.85) 100%)"></div>')
    ...
    # ---- floating doodles in the gap under the fan ----
    # v2's single heart left this whole band reading SPARSE (preview/quadrant_fill
    # flagged the bottom two quadrants) -- a lone icon in ~400px of black is a dead
    # zone, not restraint. Fill it with a small varied cluster instead of one more
    # big shape (CLAUDE.md sec.9: vary sizes, scale existing before adding filler --
    # this IS the filler, sized and placed in the one truly free band left).
    # v3's cluster was correctly PLACED (preview.critique's dead_quadrant/sparse
    # check is ground-agnostic, unlike ref_metrics.vdr/ink -- see the note in
    # friction3/g4.md) but still too SMALL: BL/BR quadrant fill stayed ~0.33-0.34
    # against ~0.55-0.61 up top. Scale the existing pieces up before adding more.
    dood_size = 172
    dood_x, dood_y = 130, 1300
    dood_col = core.on_dark(core.ACCENTS[0], 172, True)   # pink, checked against ink
    ...
    star_size = 92
    star_x, star_y = 380, 1250
    star_col = core.on_dark(LEMON, star_size, True)
    doodle2 = (f'<div class="measure" data-tag="doodle2" style="position:absolute;left:{star_x}px;'
               f'top:{star_y}px;width:{star_size}px;height:{star_size}px;z-index:9">'
               f'{dd.stamp("sparkle", star_col, rot=12)}</div>')
    elements.append(("doodle2", star_x, star_y, star_size, star_size))

    plus_size = 70
    plus_x, plus_y = 800, 1380
    plus_col = core.on_dark(TOMATO, plus_size, True)
    doodle3 = (f'<div class="measure" data-tag="doodle3" style="position:absolute;left:{plus_x}px;'
               f'top:{plus_y}px;width:{plus_size}px;height:{plus_size}px;z-index:9">'
               f'{dd.stamp("plus", plus_col, rot=0)}</div>')
    elements.append(("doodle3", plus_x, plus_y, plus_size, plus_size))

    # ---- ellipse callout near the bottom (soft CTA, no urgency, real closure) ----
    ell_w, ell_h = 920, 250
    ell_x, ell_y = (W-ell_w)//2, 1500
    ...
    inner = "".join([
        ...
        badge, doodle, doodle2, doodle3, ellipse,
        logo_pill, kicker, foot1, foot2,
    ])
    html = B.page(W, H, BG, inner, grain=True)

    # occlusion_check: does anything HIGHER-z bury a lower-z piece near-completely?
    # (badge/chip/doodles pin the fan's seams on purpose -- this confirms none of
    # them, nor the cards under them, cross from "intentional overlap" into "reads
    # as a mistake". Advisory, opt-in -- never fires unless given the z per item.)
    Z = {"cardD":6,"cardB":7,"cardC":8,"cardA":9,"chip":9,"badge":10,
         "doodle":9,"doodle2":9,"doodle3":9,"ellipse":9}
    occlusion_items = [(lbl, (x,y,w,h), Z[lbl]) for (lbl,x,y,w,h) in elements if lbl in Z]

    slug = "g4_558_welfare"
    outdir = f"out/session10g"
    os.makedirs(outdir, exist_ok=True)
    async with B.session():
        issues = await B.render(html, f"{outdir}/g4_v4.png", W, H,
