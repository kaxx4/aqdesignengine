...
async def main():
    async with B.session():
        ...
        # x = M (64) was WRONG: the photo starts at x=230, so a handle at x=64 sits mostly on
        # the plain CREAM page background, not on the photo/scrim it was colour-picked for --
        # cream-on-cream, functionally invisible. Caught only by looking at the render; no gate
        # in the stack can tell "the colour you declared for this text's surface is not what is
        # actually behind it" the way same_as_bg_scan infers page_bg automatically. Moved onto
        # the photo itself, inset from its left edge.
        handle_x, handle_y = photo_x + 24, H - 56 - handle_h
        ...
        await B.render(html, f"{outdir}/g2_v6.png", W, H, elements=elements,
        ...
        print("done -> out/session10g/g2_v6.png")
