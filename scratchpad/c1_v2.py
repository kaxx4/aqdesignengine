...
Run:  PYTHONIOENCODING=utf-8 python scratchpad/c1_v2.py
...
async def main():
    ...
    parts.append(f'<div style="position:absolute;left:{dx+52}px;top:{tag_y}px;background:{MINT};'
                 ...
                 f'border:3px solid {INK};z-index:10">welfare programme</div>')
    ...
    with open("out/session10f/c1_v2.html", "w", encoding="utf-8") as f:
        f.write(html)
    async with B.session():
        await B.render(html, "out/session10f/c1_v2.png", W, H, elements=elements,
                       color_pairs=color_pairs, page_bg=INK, expect_hero=True)
    print("done -> out/session10f/c1_v2.png")
