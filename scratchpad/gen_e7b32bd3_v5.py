...
# pile reads about this width relative to the card). v3 had RH=140 which only peeked 25px
# below the book photo -- a thin garnish stripe, not "anchors the bottom of the pile" per
# the audit; taller so it actually reads as its own object, not a rug fringe.
RX, RY, RW, RH = 575, 350, 165, 200
...
async def main():
    ...
    async with B.session():
        await B.render(html, f"{outdir}/v5.png", W, H, elements=els, color_pairs=color_pairs,
                        page_bg=CREAM, expect_hero=True, collision_ignore=frozenset(CONTAINMENT_IGNORE))
    print("done ->", f"{outdir}/v5.png")
