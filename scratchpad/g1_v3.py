"""Workflow C -- bespoke FRESH poster, v3. See g1_v1.py for the full brief/style/content
writeup -- unchanged. v2 fixed the footer collision, invisible fill, contrast and gate-
coverage gaps (see below). v3's ONLY change: the looking gate on v2 caught one more real
flaw -- the ink slab was sized 860x430, but the "126" numeral only measures 514px wide
(measure_text, size 280 var(--d)), leaving ~240px of dead flat black to the right of the
digits inside the hero itself -- a dead zone inside the one element that's supposed to be
the piece's focal point. Measured the actual numeral width and sized the slab to it
(680x430, incidentally landing at 1.58:1 -- close to the drawn reference's own 1.536:1) so
the hero card fits its content instead of floating it in a void.

v2's original changelog, still true here:
...
async def main():
    ...
    out_png = "out/session10g/g1_v3.png"
